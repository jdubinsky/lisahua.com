const STATIC_URL = process.env.STATIC_URL;

// homepage
const homepageBaseUrl = `${STATIC_URL}images/homepage/`;
export const pankoUrl = `${homepageBaseUrl}panku.jpg`;
export const tootsieUrl = `${homepageBaseUrl}tootsie.jpg`;
export const magnetUrl = `${homepageBaseUrl}desktop-magnet.png`;
export const ttcUrl = `${homepageBaseUrl}mobile-ttcexit.png`;
export const aodaUrl = `${homepageBaseUrl}desktop-aoda.png`;
export const netsweeperUrl = `${homepageBaseUrl}desktop-netsweeper.png`;
export const canvassUrl = `${homepageBaseUrl}desktop-canvass.png`;
export const oncallUrl = `${homepageBaseUrl}desktop_oncall.png`;
export const esightUrl = `${homepageBaseUrl}desktop_esight.png`;

// magnet case study images
const magnetBaseUrl = `${STATIC_URL}images/magnet/`;
export const magnetHeroUrl = `${magnetBaseUrl}hero.jpg`;
export const magnetHeroUrl2x = `${magnetBaseUrl}hero@2x.jpg`;
export const magnetStakeholdersUrl = `${magnetBaseUrl}stakeholders.jpg`;
export const magnetStakeholdersUrl2x = `${magnetBaseUrl}stakeholders@2x.jpg`;
export const magnetPersonaFE = `${magnetBaseUrl}persona-FE.jpg`;
export const magnetPersonaFE2x = `${magnetBaseUrl}persona-FE@2x.jpg`;
export const magnetPersonaPL = `${magnetBaseUrl}persona-PL.jpg`;
export const magnetPersonaPL2x = `${magnetBaseUrl}persona-PL@2x.jpg`;
export const magnetPersonaInvestigator = `${magnetBaseUrl}persona-Investigator.jpg`;
export const magnetPersonaInvestigator2x = `${magnetBaseUrl}persona-Investigator@2x.jpg`;
export const magnetJourneyMap = `${magnetBaseUrl}journey\ map.jpg`;
export const magnetJourneyMap2x = `${magnetBaseUrl}journey\ map@2x.jpg`;
export const magnetIA = `${magnetBaseUrl}IA.png`;
export const magnetIA2x = `${magnetBaseUrl}IA@2x.png`;
export const magnetWireframes = `${magnetBaseUrl}wireframes.png`;
export const magnetWireframes2x = `${magnetBaseUrl}wireframes@2x.png`;
export const magnetUT = `${magnetBaseUrl}usability\ testing.jpg`;
export const magnetUT2x = `${magnetBaseUrl}usability\ testing@2x.jpg`;
export const magnetCustomerUrl = `${magnetBaseUrl}user\ interview.jpg`;
export const magnetCustomerUrl2x = `${magnetBaseUrl}user\ interview@2x.jpg`;
export const magnetWhiteboardUrl = `${magnetBaseUrl}project\ goals.jpg`;
export const magnetWhiteboardUrl2x = `${magnetBaseUrl}project\ goals@2x.jpg`;

// files
export const resumeUrl = `${STATIC_URL}resume.pdf`;
